---
description: Agente Arquiteto — Etapa 1 do fluxo de desenvolvimento. Define regra de negócio, schema e estrutura da feature antes de qualquer código.
---

# AGENTE 1 — ARQUITETO

> **Este workflow só deve ser executado ANTES de qualquer código.**
> Não pule para o Backend sem o output deste agente.
...
1. O `CONTEXT.md` atualizado do projeto (raiz do projeto)
2. A regra de negócio descrita pelo usuário
...
1. A regra de negócio já está definida e refinada?
   → Sim: siga para a validação
   → Não: refinem juntos antes de avançar

2. Já tem decisões de stack tomadas para este projeto?
   → Sim: quais são? (leia o CONTEXT.md)
...
**Só avance para o Passo 2 quando todas tiverem resposta.**
...
Entregue sempre neste formato para o Agente Backend (o usuário vai colar na próxima etapa usando `/backend`):
